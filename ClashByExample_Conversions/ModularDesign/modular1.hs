...
run3State mp@MPIn{..} = (_done <$> s1,_done <$> s2,_done <$> s3, _kill_ltchd)
  where
    ...
    s2 = register (resetSt (St Idle 0 False False)) (runSt <$> s2 <*> (signal $ partial2full _pin2 _clk' _reset') <*> sbool)
    s3 = register (resetSt (St Idle 0 False False)) (runSt <$> s3 <*> (signal $ partial2full _pin3 _clk' _reset') <*> sbool)
